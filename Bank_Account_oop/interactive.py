...
# Interactive command-line version — takes real user input
# while still demonstrating OOP concepts (inheritance, polymorphism,
# encapsulation, abstraction, custom exceptions).
...
def create_account():
    ...
    if choice == '1':
        rate = input("Enter interest rate (default 0.04): ").strip()
        rate = float(rate) if rate else 0.04
        min_bal = input("Enter minimum balance to maintain (default 500): ").strip()
        min_bal = float(min_bal) if min_bal else 500
        account = SavingsAccount(name, balance, interest_rate=rate, min_balance=min_bal)
    elif choice == '2':
        limit = input("Enter overdraft limit (default 1000): ").strip()
...
def main():
    ...
    while True:
        ...
        if choice == '1':
            ...
            print(repr(account))    # uses __repr__
            ...
            print(f"Thank you for banking with {BANK_NAME}. Goodbye!")
            break
